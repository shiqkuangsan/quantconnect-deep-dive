# QuantConnect 深度拆解系列（架构参考手册）

> **定位：架构参考手册** — 解释 LEAN 引擎"为什么这样设计"。适合在实操过程中遇到原理性问题时回来查阅。
>
> 如果你是第一次接触 QuantConnect，建议先阅读 [团队上手指南](../quantconnect-hands-on/README.md)（"跟着做"），遇到想深入理解的部分再回到本系列（"想明白"）。
